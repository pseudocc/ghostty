    ...
    switch (write_action) {
        ...
        .pager => {
            if (self.config.pager_scheme) |scheme| {
                const SEP = "://";
                if (scheme.len > SCHEME_MAX_LEN - SEP.len) {
                    log.err("pager scheme too long", .{});
                    return;
                }
                const offset = SCHEME_MAX_LEN - scheme.len - SEP.len;
                const scheme_buf = url_buf[offset..SCHEME_MAX_LEN];
                @memcpy(scheme_buf[0..scheme.len], scheme);
                @memcpy(scheme_buf[scheme.len..], SEP);
